...
mean(fake_core) #Mean value for all values in table.
...
### Using Apply to Work Efficiently
...
fake_mode <- apply(fake_core, 2, median)
...
lines(table(fake_core$Cyperaceae.undiff.)*10,
      lty = 3,
      col = "darkred",
...
Standard deviations are the average deviation from the mean. We can calculate these manually to cement our knowledge. Here we apply another new function, abs(), which allows us to get absolute values.

```{r manual calculation, echo = TRUE}

sd_cyp <- sum(abs(fake_core$Cyperaceae.undiff. - mean(fake_core$Cyperaceae.undiff.)))/length(fake_core$Cyperaceae.undiff.)
